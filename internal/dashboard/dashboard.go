...
function drawCurve(){const c=$('curveCanvas');if(!c||!state)return;const x=c.getContext('2d'),w=c.width,h=c.height;const dark=document.body.classList.contains('dark');const fg=dark?'#94a3b8':'#64748b';const grid=dark?'rgba(51, 65, 85, 0.5)':'rgba(203, 213, 225, 0.5)';const accent=dark?'#60a5fa':'#3b82f6';const padL=46,padR=14,padT=32,padB=30;x.clearRect(0,0,w,h);x.font='500 12px "Inter", "Microsoft YaHei", sans-serif';x.strokeStyle=grid;x.lineWidth=1;x.fillStyle=fg;for(let s=0;s<=10;s+=2){const v=s*10,y=h-padB-v/100*(h-padT-padB);x.beginPath();x.moveTo(padL,y);x.lineTo(w-padR,y);x.stroke();x.textAlign='right';x.textBaseline='middle';x.fillText(v+'%',padL-8,y)}for(let t=30;t<=100;t+=10){const px=padL+(t-30)/70*(w-padL-padR);x.beginPath();x.moveTo(px,padT);x.lineTo(px,h-padB);x.stroke();x.textAlign='center';x.textBaseline='top';x.fillText(t+'°',px,h-padB+8)}const cur=state.latest&&state.latest.target_temp;if(cur!=null){const px=padL+(Math.max(30,Math.min(100,cur))-30)/70*(w-padL-padR);x.strokeStyle=dark?'#fbbf24':'#f59e0b';x.lineWidth=2;x.setLineDash([4,4]);x.beginPath();x.moveTo(px,padT);x.lineTo(px,h-padB);x.stroke();x.setLineDash([])}const curve=state.config.curve;const pts=curve.map(p=>[padL+(p[0]-30)/70*(w-padL-padR),h-padB-p[1]/100*(h-padT-padB)]);x.strokeStyle=accent;x.lineWidth=3;x.lineJoin='round';x.beginPath();pts.forEach(([px,py],i)=>{if(i===0)x.moveTo(px,py);else x.lineTo(px,py)});x.stroke();const grad=x.createLinearGradient(0,padT,0,h-padB);grad.addColorStop(0,accent.replace(')',', 0.3)').replace('rgb','rgba').replace('#3b82f6','rgba(59,130,246,0.3)').replace('#60a5fa','rgba(96,165,250,0.3)'));grad.addColorStop(1,accent.replace(')',', 0)').replace('rgb','rgba').replace('#3b82f6','rgba(59,130,246,0)').replace('#60a5fa','rgba(96,165,250,0)'));x.lineTo(pts[pts.length-1][0],h-padB);x.lineTo(pts[0][0],h-padB);x.closePath();x.fillStyle=grad;x.fill();pts.forEach(([px,py],i)=>{x.fillStyle=accent;x.beginPath();x.arc(px,py,8,0,Math.PI*2);x.fill();x.strokeStyle=dark?'#1e293b':'#ffffff';x.lineWidth=3;x.stroke();x.fillStyle=fg;x.font='600 11px "Inter", "Microsoft YaHei", sans-serif';x.textAlign='center';x.textBaseline='bottom';x.fillText(curve[i][0]+'°/'+curve[i][1]+'%',px,py-18)});const vals=$('curveValues');if(vals)vals.textContent=curve.map(p=>'('+p[0]+'°, '+p[1]+'%)').join(' · ')}
